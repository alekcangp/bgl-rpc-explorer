...
if (
  ...
  !rpcCred.username &&
  !rpcCred.password &&
  fs.existsSync(rpcCred.cookie)
) {
  console.log(`Loading RPC cookie file: ${rpcCred.cookie}`);

  [rpcCred.username, rpcCred.password] = fs
    .readFileSync(rpcCred.cookie)
    .toString()
    .split(":", 2);
...
const cookieSecret =
  process.env.BTCEXP_COOKIE_SECRET ||
  (rpcCred.password &&
    crypto
      .createHmac("sha256", JSON.stringify(rpcCred))
      .update("btc-rpc-explorer-cookie-secret")
      .digest("hex")) ||
  "0x000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f";

const electrumServerUriStrings = (
  process.env.BTCEXP_ELECTRUM_SERVERS ||
  process.env.BTCEXP_ELECTRUMX_SERVERS ||
  ""
)
  .split(",")
  .filter(Boolean);
...
["BTCEXP_NO_RATES", "BTCEXP_SLOW_DEVICE_MODE"].forEach(function (item) {
  if (process.env[item] === undefined) {
...
const slowDeviceMode =
  process.env.BTCEXP_SLOW_DEVICE_MODE.toLowerCase() == "true";
...
module.exports = {
  ...
  queryExchangeRates:
    process.env.BTCEXP_NO_RATES.toLowerCase() != "true" &&
    process.env.BTCEXP_PRIVACY_MODE.toLowerCase() != "true",
  noInmemoryRpcCache:
    process.env.BTCEXP_NO_INMEMORY_RPC_CACHE.toLowerCase() == "true",

  rpcConcurrency:
    process.env.BTCEXP_RPC_CONCURRENCY || (slowDeviceMode ? 3 : 10),

  filesystemCacheDir:
    process.env.BTCEXP_FILESYSTEM_CACHE_DIR ||
  ...
  rpcBlacklist:
    process.env.BTCEXP_RPC_ALLOWALL.toLowerCase() == "true"
      ? []
      : process.env.BTCEXP_RPC_BLACKLIST
      ? process.env.BTCEXP_RPC_BLACKLIST.split(",").filter(Boolean)
      : [
          "addnode",
          "backupwallet",
          "bumpfee",
  ...
  site: {
    ...
    homepage: {
      recentBlocksCount: parseInt(
        process.env.BTCEXP_UI_HOME_PAGE_LATEST_BLOCKS_COUNT ||
          (slowDeviceMode ? 5 : 10)
      )
    },
    ...
    browseBlocksPageSize: parseInt(
      process.env.BTCEXP_UI_BLOCKS_PAGE_BLOCK_COUNT ||
        (slowDeviceMode ? 10 : 25)
    ),
  ...
  siteTools: [
    /* 0 */ {
      name: "Node Details",
      url: "./node-details",
      desc: "Node basics (version, uptime, etc)",
      iconClass: "bi-info-circle"
    },
    /* 1 */ {
      name: "Peers",
      url: "./peers",
      desc: "Details about the peers connected to this node.",
    ...
    /* 6 */ {
      ...
      url: "./rpc-browser",
      desc:
        "Browse the RPC functionality of this node. See docs and execute commands.",
    ...
    /* 8 	*/ {
      ...
      url: "./fun",
      desc: "Curated fun/interesting historical blockchain data.",
      iconClass: "bi-flag"
    },
    ...
    /* 9 */ {
      name: "Mining Summary",
      url: "./mining-summary",
      desc: "Summary of recent data about miners.",
      iconClass: "bi-hammer"
    },
    /* 10 */ {
      name: "Block Stats",
      url: "./block-stats",
      desc: "Summary data for blocks in configurable range.",
      iconClass: "bi-stack"
    },
    /* 11 */ {
      name: "Block Analysis",
      url: "./block-analysis",
      desc: "Summary analysis for all transactions in a block.",
      iconClass: "bi-chevron-double-down"
    },
    /* 12 */ {
      name: "Difficulty History",
      url: "./difficulty-history",
      desc: "Details of difficulty changes over time.",
      iconClass: "bi-clock-history"
    },

    /* 13 */ {
      name: "Whitepaper Extractor",
      url: "./bitcoin-whitepaper",
      desc:
        "Extract the Bitcoin whitepaper from data embedded within the blockchain.",
      iconClass: "bi-file-earmark-text"
    },

    /* 14 */ {
      name: "Predicted Blocks",
      url: "./predicted-blocks",
      desc: "View predicted future blocks based on the current mempool.",
    ...
    /* 15 */ {
      name: "API",
      url: "./api/docs",
      desc: "View docs for the public API.",
    ...
    /* 16 */ {
      ...
      url: "./next-block",
      desc:
        "View a prediction for the the next block, based on the current mempool.",
      iconClass: "bi-minecart-loaded"
    },
    /* 17 */ {
      name: "Quotes",
      url: "./quotes",
      desc: "Curated list of Bitcoin-related quotes.",
    ...
    /* 19 */ {
      name: "Holidays",
      url: "./holidays",
      desc: "Curated list of Bitcoin 'Holidays'.",
...
debugLog(
  `Config(final): rpcAllowAll=${module.exports.rpcBlacklist.length == 0}`
);
