...
var noTxIndexMsg =
  "\n\nYour node does not have **txindex** enabled. Without it, you can only lookup wallet, mempool, and recently confirmed transactions by their **txid**. Searching for non-wallet transactions that were confirmed more than " +
  config.noTxIndexSearchDepth +
  " blocks ago is only possible if the confirmed block height is available.";
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      const { perfId, perfResults } = utils.perfLogNewItem({
        action: "homepage"
      });
      ...
      promises.push(
        utils.timePromise(
          "homepage.getSmartFeeEstimates",
          async () => {
            const rawSmartFeeEstimates = await coreApi.getSmartFeeEstimates(
              "CONSERVATIVE",
              feeConfTargets
            );
            ...
            for (var i = 0; i < feeConfTargets.length; i++) {
              ...
              if (rawSmartFeeEstimate.errors) {
                ...
                smartFeeEstimates[feeConfTargets[i]] = parseInt(
                  new Decimal(rawSmartFeeEstimate.feerate)
                    .times(coinConfig.baseCurrencyUnit.multiplier)
                    .dividedBy(1000)
                );
              }
            }
      ...
      res.locals.difficultyPeriod = parseInt(
        Math.floor(
          getblockchaininfo.blocks / coinConfig.difficultyAdjustmentBlockCount
        )
      );
      ...
      promises.push(
        utils.timePromise(
          "homepage.getBlocksStatsByHeight",
          async () => {
            const rawblockstats = await coreApi.getBlocksStatsByHeight(
              blockHeights
            );

            if (
              rawblockstats &&
              rawblockstats.length > 0 &&
              rawblockstats[0] != null
            ) {
      ...
      promises.push(
        utils.timePromise(
          "homepage.getBlockHeaderByHeight",
          async () => {
            let h =
              coinConfig.difficultyAdjustmentBlockCount *
              res.locals.difficultyPeriod;
            res.locals.difficultyPeriodFirstBlockHeader = await coreApi.getBlockHeaderByHeight(
              h
            );
          },
          perfResults
      ...
      promises.push(
        utils.timePromise("homepage.getBlocksByHeight", async () => {
          const latestBlocks = await coreApi.getBlocksByHeight(blockHeights);

          res.locals.latestBlocks = latestBlocks;
          res.locals.blocksUntilDifficultyAdjustment =
            (res.locals.difficultyPeriod + 1) *
              coinConfig.difficultyAdjustmentBlockCount -
            latestBlocks[0].height;
        })
      );

      var targetBlocksPerDay =
        (24 * 60 * 60) / global.coinConfig.targetBlockTimeSeconds;
      ...
      if (false && getblockchaininfo.chain !== "regtest") {
        ...
        var chainTxStatsIntervals = [
          [targetBlocksPerDay, "24 hours"],
          [7 * targetBlocksPerDay, "7 days"],
          [30 * targetBlocksPerDay, "30 days"]
        ...
        for (var i = 0; i < chainTxStatsIntervals.length; i++) {
          promises.push(
            utils.timePromise(
              `homepage.getChainTxStats.${chainTxStatsIntervals[i][0]}`,
              async () => {
                res.locals.chainTxStats[
                  chainTxStatsIntervals[i][0]
                ] = await coreApi.getChainTxStats(chainTxStatsIntervals[i][0]);
              },
              perfResults
            )
          );
        ...
        promises.push(
          utils.timePromise(
            "homepage.getChainTxStats.allTime",
            async () => {
              res.locals.chainTxStats[-1] = await coreApi.getChainTxStats(
                getblockchaininfo.blocks - 1
              );
            },
            perfResults
          )
      ...
      var daysUntilAdjustment = new Decimal(
        res.locals.blocksUntilDifficultyAdjustment
      )
        .times(timePerBlock)
        .dividedBy(60 * 60 * 24);
      var hoursUntilAdjustment = new Decimal(
        res.locals.blocksUntilDifficultyAdjustment
      )
        .times(timePerBlock)
        .dividedBy(60 * 60);
      var duaDP1 = daysUntilAdjustment.toDP(1);
      var daysUntilAdjustmentStr =
        daysUntilAdjustment > 1
          ? `~${duaDP1} day${duaDP1 == "1" ? "" : "s"}`
          : "< 1 day";
      var hoursUntilAdjustmentStr =
        hoursUntilAdjustment > 1
          ? `~${hoursUntilAdjustment.toDP(0)} hr${
              hoursUntilAdjustment.toDP(1) == "1" ? "" : "s"
            }`
          : "< 1 hr";
      ...
      var blockRatioPercent = new Decimal(
        blockCount / predictedBlockCount
      ).times(100);
      ...
      if (predictedBlockCount > blockCount) {
        var diffAdjPercent = new Decimal(100)
          .minus(blockRatioPercent)
          .times(-1);
        var diffAdjText = `Blocks during the current difficulty epoch have taken this long, on average, to be mined. If this pace continues, then in ${res.locals.blocksUntilDifficultyAdjustment.toLocaleString()} block${
          res.locals.blocksUntilDifficultyAdjustment == 1 ? "" : "s"
        } (${daysUntilAdjustmentStr}) the difficulty will adjust downward: -${diffAdjPercent.toDP(
          1
        )}%`;
        var diffAdjSign = "-";
        ...
        var diffAdjText = `Blocks during the current difficulty epoch have taken this long, on average, to be mined. If this pace continues, then in ${res.locals.blocksUntilDifficultyAdjustment.toLocaleString()} block${
          res.locals.blocksUntilDifficultyAdjustment == 1 ? "" : "s"
        } (${daysUntilAdjustmentStr}) the difficulty will adjust upward: +${diffAdjPercent.toDP(
          1
        )}%`;
        var diffAdjSign = "+";
      ...
      res.locals.difficultyAdjustmentData = {
        ...
        timeLeftStr:
          daysUntilAdjustment < 1
            ? hoursUntilAdjustmentStr
            : daysUntilAdjustmentStr,
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      promises.push(
        utils.timePromise(
          "node-details.getBlockchainInfo",
          async () => {
      ...
      promises.push(
        utils.timePromise(
          "node-details.getDeploymentInfo",
          async () => {
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      res.locals.satoshiPerByteBucketMaxima =
        coinConfig.feeSatoshiPerByteBucketMaxima;
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      if (peerIps.length > 0) {
        res.locals.peerIpSummary = await utils.timePromise(
          "peers.geoLocateIpAddresses",
          async () => {
        ...
        res.locals.mapBoxComApiAccessKey =
          config.credentials.mapBoxComApiAccessKey;
      }
...
router.post("/connect", function (req, res, next) {
  ...
  req.session.userMessage =
    "<span class='font-weight-bold'>Connected via RPC</span>: " +
    username +
    " @ " +
    host +
    ":" +
...
router.get("/session-data", function (req, res, next) {
  if (req.query.action && req.query.data) {
    ...
    if (action == "remove-rpc-favorite") {
      ...
      if (req.session.favoriteRpcCommands.includes(data)) {
        req.session.favoriteRpcCommands.splice(
          req.session.favoriteRpcCommands.indexOf(data),
          1
        );
      }
    }
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      var getblockchaininfo = await utils.timePromise(
        "blocks.geoLocateIpAddresses",
      ...
      if (sort == "desc") {
        for (
          var i = getblockchaininfo.blocks - offset;
          i > getblockchaininfo.blocks - offset - limit - 1;
          i--
        ) {
          if (i >= 0) {
            blockHeights.push(i);
      ...
      promises.push(
        utils.timePromise(
          ...
          async () => {
            try {
              var rawblockstats = await coreApi.getBlocksStatsByHeight(
                blockHeights
              );
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      var getblockchaininfo = await utils.timePromise(
        "mining-summary.getBlockchainInfo",
        coreApi.getBlockchainInfo
      );
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      res.locals.metaTitle = `Extended Public Key: ${utils.ellipsizeMiddle(
        extendedPubkey,
        24
      )}`;
      ...
      if (extendedPubkey.match(/^(xpub|tpub).*$/)) {
        ...
        res.locals.receiveAddresses = utils.bip32Addresses(
          extendedPubkey,
          "p2pkh",
          0,
          limit,
        ...
        res.locals.changeAddresses = utils.bip32Addresses(
          extendedPubkey,
          "p2pkh",
          1,
          limit,
        ...
        res.locals.pubkeyTypeDesc =
          "Pay to Witness Public Key Hash (P2WPKH) wrapped inside Pay to Script Hash (P2SH), aka Wrapped Segwit";
        ...
        res.locals.pubkeyTypeDesc =
          "Pay to Witness Public Key Hash, aka Native Segwit";
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    if (semver.lt(global.btcNodeSemver, rpcApi.minRpcVersions.getblockstats)) {
      res.locals.rpcApiUnsupportedError = {
        rpc: "getblockstats",
        version: rpcApi.minRpcVersions.getblockstats
      };
    }
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    blockTemplate.transactions.forEach((tx) => {
      ...
      // into account)
      if (
        !parentTxIndexes.has(txIndex) &&
        (!tx.depends || tx.depends.length == 0)
      ) {
...
router.post("/search", function (req, res, next) {
  if (!req.body.query) {
    req.session.userMessage =
      "Enter a block height, block hash, or transaction id.";
  ...
  if (query.length == 64) {
    ...
    coreApi
      .getBlockByHeight(parseInt(query))
      .then(function (blockByHeight) {
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      if (req.query.limit) {
        ...
        if (config.demoSite && limit > config.site.blockTxPageSize) {
          ...
          res.locals.userMessage =
            "Transaction page size limited to " +
            config.site.blockTxPageSize +
            ". If this is your site, you can change or disable this limit in the site config.";
        }
      }
      ...
      const result = await utils.timePromise(
      ...
      promises.push(
        utils.timePromise(
          "block-height.getBlockByHashWithTransactions",
          async () => {
            const blockWithTransactions = await coreApi.getBlockByHashWithTransactions(
              result.hash,
              limit,
              offset
            );
            ...
            res.locals.result.txInputsByTransaction =
              blockWithTransactions.txInputsByTransaction;
          },
          perfResults
        )
      );
      ...
      if (
        ...
        global.specialBlocks[res.locals.result.getblock.hash]
      ) {
        ...
        if (funInfo.alertBodyHtml) {
          res.locals.metaDesc = funInfo.alertBodyHtml.replace(
            /<\/?("[^"]*"|'[^']*'|[^>])*(>|$)/g,
            ""
          );
        } else {
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      promises.push(
        utils.timePromise(
          "block.getBlockByHashWithTransactions",
          async () => {
            const blockWithTransactions = await coreApi.getBlockByHashWithTransactions(
              blockHash,
              limit,
          ...
          async () => {
            try {
              ...
              if (global.prunedBlockchain) {
                // unavailable, likely due to pruning
                debugLog(
                  "Failed loading block stats, likely due to pruning",
                  err
                );
              } else {
                throw err;
      ...
      if (
        ...
        res.locals.metaTitle = `Bitcoin Block ${utils.ellipsizeMiddle(
          res.locals.result.getblock.hash,
          16
        )}`;
        res.locals.metaDesc = "";
      }
...
router.get(
  "/predicted-blocks-old",
  asyncHandler(async (req, res, next) => {
    try {
      const mempoolTxids = await utils.timePromise(
        "predicted-blocks.getAllMempoolTxids",
        coreApi.getAllMempoolTxids()
      );
      let mempoolTxSummaries = await coreApi.getMempoolTxSummaries(
        mempoolTxids,
      ...
      for (var i = 0; i < mempoolTxSummaries.length; i++) {
        ...
        if (currentBlock.weight + tx.w > coinConfig.maxBlockWeight) {
          ...
          currentBlock.avgFee = currentBlock.totalFees.dividedBy(
            currentBlock.txCount
          );
          currentBlock.avgFeeRate = currentBlock.totalFees.dividedBy(
            currentBlock.vB
          );
...
router.get("/block-analysis/:blockHashOrHeight", function (req, res, next) {
  ...
  if (!isNaN(blockHashOrHeight)) {
    coreApi
      .getBlockByHeight(parseInt(blockHashOrHeight))
      .then(function (blockByHeight) {
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      var txPromise = req.query.blockHeight
        ? async () => {
            const block = await coreApi.getBlockByHeight(
              parseInt(req.query.blockHeight)
            );
            res.locals.block = block;
            return await coreApi.getRawTransactionsWithInputs(
              [txid],
              txInputLimit,
              block.hash
            );
      ...
      const rawTxResult = await utils.timePromise(
        "tx.getRawTransactionsWithInputs",
        txPromise,
        perfResults
      );
      ...
      res.locals.result.txInputs =
        rawTxResult.txInputsByTransaction[txid] || {};
      ...
      if (tx.confirmations == null) {
        promises.push(
          utils.timePromise(
            "tx.getMempoolTxDetails",
            async () => {
              res.locals.mempoolDetails = await coreApi.getMempoolTxDetails(
                txid,
                true
              );
            },
        ...
        promises.push(
          utils.timePromise(
            "tx.getblockheader",
            async () => {
              res.locals.result.getblock = await global.rpcClient.command(
                "getblockheader",
                tx.blockhash
              );
            },
      ...
      if (global.specialTransactions && global.specialTransactions[txid]) {
        ...
        res.locals.metaTitle = `Bitcoin Transaction ${utils.ellipsizeMiddle(
          txid,
          16
        )}`;
        res.locals.metaDesc = "";
      ...
      if (
        global.prunedBlockchain &&
        res.locals.blockHeight &&
        res.locals.blockHeight < global.pruneHeight
      ) {
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      res.locals.maxTxOutputDisplayCount =
        config.site.addressPage.txOutputMaxDefaultDisplay;
      ...
      let b58prefix =
        global.activeBlockchain == "main" ? /^[13].*$/ : /^[2mn].*$/;
      if (address.match(b58prefix)) {
        try {
          res.locals.addressObj = bitcoinjs.address.fromBase58Check(address);
          res.locals.addressObj.hash = res.locals.addressObj.hash.toString(
            "hex"
          );
      ...
      if (addressEncoding == "unknown") {
        try {
          ...
          res.locals.addressObj.data = res.locals.addressObj.data.toString(
            "hex"
          );
      ...
      if (addressEncoding == "unknown") {
        try {
          ...
          res.locals.addressObj.words = Buffer.from(
            res.locals.addressObj.words
          ).toString("hex");
      ...
      if (global.miningPoolsConfigs) {
        for (var i = 0; i < global.miningPoolsConfigs.length; i++) {
          if (global.miningPoolsConfigs[i].payout_addresses[address]) {
            res.locals.payoutAddressForMiner =
      ...
      if (!res.locals.crawlerBot) {
        var addrScripthash = hexEnc.stringify(
          sha256(hexEnc.parse(validateaddressResult.scriptPubKey))
        );
              ...
              if (addressDetails) {
                ...
                if (addressDetails.txids) {
                  ...
                  const rawTxResult = await (global.txindexAvailable
                    ...
                    : coreApi
                        .getRawTransactionsByHeights(txids, blockHeightsByTxid)
                        .then((transactions) => ({
                          transactions,
                  ...
                  res.locals.txInputsByTransaction =
                    rawTxResult.txInputsByTransaction;
                  ...
                  if (coinbaseTxs.length > 0) {
                    ...
                    blockHeightsPromises.push(
                      utils.timePromise(
                        "address.getBlocksByHash",
                        async () => {
                          const blocksByHashResult = await coreApi.getBlocksByHash(
                            coinbaseTxBlockHashes
                          );
                          for (var txid in blockHashesByTxid) {
                            if (blockHashesByTxid.hasOwnProperty(txid)) {
                              blockHeightsByTxid[txid] =
                                blocksByHashResult[
                                  blockHashesByTxid[txid]
                                ].height;
                            }
                          }
                        },
                  ...
                  for (var i = 0; i < rawTxResult.transactions.length; i++) {
                    var tx = rawTxResult.transactions[i];
                    var txInputs =
                      rawTxResult.txInputsByTransaction[tx.txid] || {};
                    ...
                    for (var j = 0; j < tx.vout.length; j++) {
                      if (tx.vout[j].value > 0 && tx.vout[j].scriptPubKey) {
                        if (
                          utils.getVoutAddresses(tx.vout[j]).includes(address)
                        ) {
                          if (addrGainsByTx[tx.txid] == null) {
                            addrGainsByTx[tx.txid] = new Decimal(0);
                          }

                          addrGainsByTx[tx.txid] = addrGainsByTx[tx.txid].plus(
                            new Decimal(tx.vout[j].value)
                          );
                        }
                      }
                    }
                    ...
                    for (var j = 0; j < tx.vin.length; j++) {
                      ...
                      if (txInput != null) {
                        if (txInput && txInput.scriptPubKey) {
                          if (
                            utils.getVoutAddresses(txInput).includes(address)
                          ) {
                            if (addrLossesByTx[tx.txid] == null) {
                              addrLossesByTx[tx.txid] = new Decimal(0);
                            }

                            addrLossesByTx[tx.txid] = addrLossesByTx[
                              tx.txid
                            ].plus(new Decimal(txInput.value));
                          }
                        }
                      }
                    }
      ...
      res.locals.pageErrors.push(
        utils.logError("2108hs0gsdfe", e, { address: address })
      );
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    try {
      const result = await global.rpcClientNoTimeout.command([
        { method: cmd, parameters: parsedParams }
      ...
      if (result) {
        ...
        res.write(
          JSON.stringify({ Error: "No response from node" }, null, 4),
          function () {
            res.end();
          }
        );
  ...
  asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      if (req.query.method) {
        ...
        const methodHelp = await coreApi.getRpcMethodHelp(
        ...
        if (req.query.execute) {
            ...
            for (var i = 0; i < req.query.args.length; i++) {
              ...
              for (var j = 0; j < argProperties.length; j++) {
                if (argProperties[j] === "numeric") {
                  ...
                  argProperties[j] === "string" ||
                  argProperties[j] === "numeric or string" ||
                  argProperties[j] === "string or numeric"
                ) {
                  if (req.query.args[i]) {
                  ...
                  argProperties[j] === "array" ||
                  argProperties[j] === "json array"
                ) {
                  if (req.query.args[i]) {
          ...
          try {
            ...
            const result = await global.rpcClientNoTimeout.command([
              { method: req.query.method, parameters: argValues }
            ...
            res.locals.pageErrors.push(
              utils.logError("23roewuhfdghe", err, {
                method: req.query.method,
                params: argValues
              })
            );
      ...
      res.locals.pageErrors.push(
        utils.logError("23ewyf0weee", err, {
          method: method,
          params: argValues
        })
      );
...
router.post("/terminal", function (req, res, next) {
  ...
  if (cmd == "parsescript") {
    ...
    res.write(
      JSON.stringify({ Error: "Unknown command" }, null, 4),
      function () {
        res.end();
      }
    );
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      const mempoolData = await utils.timePromise(
        "mempool-tx.getMempoolTxids",
        async () => {
      ...
      promises.push(
        utils.timePromise(
          "mempool-tx.getRawTransactionsWithInputs",
          async () => {
            const transactionData = await coreApi.getRawTransactionsWithInputs(
              txids,
              config.slowDeviceMode ? 3 : 5
            );

            res.locals.transactions = transactionData.transactions;
            res.locals.txInputsByTransaction =
              transactionData.txInputsByTransaction;
          },
          perfResults
        )
      );
      ...
      txids.forEach((txid) => {
        promises.push(
          utils.timePromise(
            "mempool-tx.getRawTransactionsWithInputs",
            async () => {
              const mempoolTxidDetails = await coreApi.getMempoolTxDetails(
                txid,
                false
              );
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    promises.push(
      utils.timePromise(
        "tx-stats.getTxStats-week",
        async () => {
          const statsWeek = await coreApi.getTxStats(
            200,
            height - 144 * 7,
            height
          );
    ...
    promises.push(
      utils.timePromise(
        "tx-stats.getTxStats-month",
        async () => {
          const statsMonth = await coreApi.getTxStats(
            250,
            height - 144 * 30,
            height
          );
    ...
    promises.push(
      utils.timePromise(
        "tx-stats.getTxStats-year",
        async () => {
          const statsYear = await coreApi.getTxStats(
            250,
            height - 144 * 365,
            height
          );
...
router.get("/difficulty-history", function (req, res, next) {
  coreApi
    .getBlockchainInfo()
    .then(function (getblockchaininfo) {
      res.locals.blockCount = getblockchaininfo.blocks;
...
router.get("/bitcoin.pdf", function (req, res, next) {
  ...
  const whitepaperTxid =
    "54e48e5f5c656b26c3bca14a8c95aa583d07ebe84dde3b7dd4a78f4e4186e713";

  // get all outputs except the last 2 using `gettxout`
  Promise.all(
    [...Array(946).keys()].map((vout) => coreApi.getTxOut(whitepaperTxid, vout))
  )
    .then(function (vouts) {
