    ...
    .then(function (results) {
      ...
      if (txInputs[0]) {
        for (let key in txInputs) {
          ...
          inputBtc +=
            item["value"] * global.coinConfig.baseCurrencyUnit.multiplier;
      ...
      outJson.fee = {
        amount:
          (inputBtc - outputBtc) /
          global.coinConfig.baseCurrencyUnit.multiplier,
        unit: "BGL"
      };
    ...
    try {
      ...
      if (address.match(/^[132mn].*$/)) {
        try {
          ...
          result.base58 = {
            hash: base58Data.hash.toString("hex"),
            version: base58Data.version
          };
      ...
      if (addressEncoding == "unknown") {
        try {
          ...
          result.bech32 = {
            data: bech32Data.data.toString("hex"),
            version: bech32Data.version
          };
      ...
      if (addressEncoding == "unknown") {
        try {
          ...
          result.bech32m = {
            words: Buffer.from(bech32mData.words).toString("hex"),
            version: bech32mData.version
          };
      ...
      result.notes = [];
      if (
        global.specialAddresses[address] &&
        global.specialAddresses[address].type == "fun"
      ) {
      ...
      promises.push(
        utils.timePromise(
          ...
          async () => {
            ...
            if (
              addressDetailsResult.errors &&
              addressDetailsResult.errors.length > 0
            ) {
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      keyDetails.receiveAddresses = xyzpubApi.getXpubAddresses(
        extendedPubkey,
        0,
        limit,
        offset
      );
      keyDetails.changeAddresses = xyzpubApi.getXpubAddresses(
        extendedPubkey,
        1,
        limit,
        offset
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    try {
      ...
      const searchResult = await xyzpubApi.searchXpubTxids(
        extendedPubkey,
        gapLimit,
        limit
      );
...
router.get(
  "/xyzpub/addresses/:extendedPubkey",
  asyncHandler(async (req, res, next) => {
    try {
      const extendedPubkey = req.params.extendedPubkey;
      ...
      const xyzpubResult = await xyzpubApi.getXpubAddresses(
        extendedPubkey,
        receiveOrChange,
        limit,
        offset
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    const { perfId, perfResults } = utils.perfLogNewItem({
      action: "api.diff-adj-estimate"
    });
    res.locals.perfId = perfId;

    var promises = [];
    const getblockchaininfo = await utils.timePromise(
      "api_diffAdjEst_getBlockchainInfo",
      coreApi.getBlockchainInfo
    );
    var currentBlock;
    var difficultyPeriod = parseInt(
      Math.floor(
    ...
    promises.push(
      utils.timePromise(
        "api.diff-adj-est.getBlockHeaderByHeight",
        async () => {
          currentBlock = await coreApi.getBlockHeaderByHeight(
    ...
    promises.push(
      utils.timePromise(
        "api.diff-adj-est.getBlockHeaderByHeight2",
        async () => {
          let h = coinConfig.difficultyAdjustmentBlockCount * difficultyPeriod;
          difficultyPeriodFirstBlockHeader = await coreApi.getBlockHeaderByHeight(
            h
          );
        },
        perfResults
    ...
    var blockRatioPercent = new Decimal(blockCount / predictedBlockCount).times(
      100
    );
...
router.get(
  "/mining/next-block/includes/:txid",
  asyncHandler(async (req, res, next) => {
    ...
    promises.push(
      utils.timePromise("api/next-block/getblocktemplate", async () => {
        ...
        for (
          let i = 0;
          i < nextBlockEstimate.blockTemplate.transactions.length;
          i++
        ) {
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (startHeight == -1 || endHeight == -1) {
      res.json({
        success: false,
        error:
          "Unknown start or end height - use either 'since' (e.g. 'since=7d') or 'startHeight'+'endHeight' parameters to specify the blocks to analyze."
      });

      return;
    }

    const summary = await coreApi.buildMiningSummary(
      null,
      startHeight,
      endHeight,
      null
...
router.get("/mempool/fees", function (req, res, next) {
  ...
  coreApi
    .getSmartFeeEstimates("CONSERVATIVE", feeConfTargets)
    .then(function (rawSmartFeeEstimates) {
...
router.get("/price/:currency/sats", function (req, res, next) {
  ...
  if (global.exchangeRates != null && global.exchangeRates[currency] != null) {
    ...
    dec = dec
      .times(global.exchangeRates.usd)
      .dividedBy(global.goldExchangeRates.usd);
...
router.get("/price/:currency/marketcap", function (req, res, next) {
  ...
  coreApi
    ...
    .then(function (getblockchaininfo) {
      ...
      if (
        global.exchangeRates != null &&
        global.exchangeRates[currency] != null
      ) {
